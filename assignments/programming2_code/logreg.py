...
import random
from numpy import zeros, sign, fabs, argpartition
...
class LogReg:
    def __init__(self, num_features, mu, step=lambda x: x):
        """
    ...
    def sg_update(self, train_example, iteration, use_tfidf=False):
        ...
        delta = train_example.x * (train_example.y - p) * self.step(iteration)
        self.beta += delta

        if self.mu > 0:
            shrinkage = (1 - 2*(self.step(iteration) * self.mu))
            self.beta[0] *= shrinkage
            for feat in train_example.nonzero:
                gap = iteration - self.last_update.get(feat, -1)
                self.beta[feat] *= pow(shrinkage,gap)
...
if __name__ == "__main__":
    ...
    for pp in xrange(args.passes):
        ...
        best = argpartition(fabs(lr.beta), -10)[-10:]
        worst = argpartition(fabs(lr.beta), 10)[:10]

        print("Best Predictors \t Beta Values")
        for ii in best:
            print("%s\t\t\t%.10g" % (vocab[ii], lr.beta[ii]))

        print("Worst Predictors \t Beta Values")
        for ii in worst:
            print("%s\t\t\t%.20g" % (vocab[ii], lr.beta[ii]))
